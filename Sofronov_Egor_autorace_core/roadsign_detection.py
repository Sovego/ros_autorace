...
class SignDetector:
    ...
    def __init__(self, path_to_signs_imgs, crop_size=128, metrics=["ppa"], score_function=None, debug_mode=False):
        self.sign_templates_paths = list(paths.list_images(path_to_signs_imgs))
    ...
    def detect(self, image, d_image):   
        ...
        #Делаем маску
        mask = cv.inRange(hsv,(94,182,0),(255,255,255)) #(92,88,0)
        #cv.imshow('Mask', mask) 
        ...
        if contours:
            ...
            dst = self.is_valid_distance(d_image_crop,treshhold=0.7)
            #print(dst)

            if self.debug_mode:
                dst_color = (0, 255, 0) if dst[1] else (255, 255, 255)
                cv.putText(image, str(dst), (500, 14), cv.FONT_HERSHEY_SIMPLEX, 0.5, dst_color, 2)
                #cv.imshow('Runtime', image)
    ...
    def classify(self, image, bbox):
        ...
        image_crop = cv.inRange(image_crop, (0,10,101), (255,255,255))
    ...
    def is_valid_distance(self, image, treshhold=0.7):
        #image = cv.split(image)[0] #убрать потом
        img_mean = image.mean()
        #image[self.CROP_SIZE//2,self.CROP_SIZE//2] 
        out = (img_mean,True) if img_mean < treshhold else (img_mean,False)
